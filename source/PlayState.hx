...
class PlayState extends MusicBeatState
{
	...
	public var fourByThree:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			//just shit for cache, all stage stuff is done through lua
			case 'tooslowstage' | 'tooslowstageencore':
				var jumps:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('sonicJUMPSCARE'));
				var daSTAT:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('daSTAT'));
				var hitStatic:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('hitStatic'));

			case 'ycrstage':
				var hitStatic:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('hitStatic'));
				var daSTAT:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('daSTAT'));
				var green:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('GreenHill'));

			case 'ttstage':
				var hitStatic:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('hitStatic'));
				var daSTAT:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('daSTAT'));
				var tail:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('Tails'));
				var knuc:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('Knuckles'));
				var egg:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('Eggman'));

			case 'fakerlol':
				var daSTAT:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('daSTAT'));
			
			case 'iwannasunksuperbad' :
				//this lone sprite being cached cause its the only lua sprite added through script in that song.
				var daSTAT:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('sunker'));
				//taken (and a little modified) from the source of 2.5 cause idk how tweening images works
				cereal = new FlxSprite(-1000, 0).loadGraphic(Paths.image("cereal"));
				cereal.cameras = [camOther];
				cereal.screenCenter(Y);
				add(cereal);

				munch = new FlxSprite(-1000, 0).loadGraphic(Paths.image("sunkyMunch"));
				munch.cameras = [camOther];
				munch.screenCenter(Y);
				add(munch);

				pose = new FlxSprite(-1000, 0).loadGraphic(Paths.image("sunkyPose"));
				pose.cameras = [camOther];
				pose.screenCenter(Y);
				add(pose);

				var funnyAspect:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image("stinkyshit"));
				funnyAspect.screenCenter();
				funnyAspect.cameras = [camOther];
				add(funnyAspect);

				spoOoOoOky = new FlxSprite(0, 0).loadGraphic(Paths.image("sunkage"));
        ...
        {
			botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "UR TO SLOW", 32);
			...
			botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "FUN IS INFINITE", 32);
		...
		else if (SONG.song.toLowerCase() == 'fatality') {
			botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "ERR 404", 32);
			botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			botplayTxt.scrollFactor.set();
			botplayTxt.borderSize = 1.25;
			botplayTxt.visible = cpuControlled;
			...
			botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "I AM GOD", 32);
			botplayTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			botplayTxt.scrollFactor.set();
			botplayTxt.borderSize = 1.25;
			botplayTxt.visible = cpuControlled;
	...
	//had to use source due to the way the splash screen at the start of the song works
	//not perfect but it will do
	function tooSlowCutscene()
	{
		var cutsceneHandler:CutsceneHandler = new CutsceneHandler();
...
public function updateScore(miss:Bool = false)
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'sunshine')
		{
				...
				//Stupid Fucking effect so god damn difficult to get working im actually pissed rn sombody HELP ME
		}
        ...
        if (SONG.song.toLowerCase() == 'fight or flight')
        {
			...
			iconP1.y = healthBar.y + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 60, 68, 0) * 0.01)) + (150 * iconP1.scale.y - 150) / 2 - iconOffset;
			iconP2.y = healthBar.y + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 60, 68, 0) * 0.01)) - (150 * iconP2.scale.y) / 2 - iconOffset * 2;
        } else {
	...
	//had to pull from source to tween objects cause im still learning this shit lol
	if (curStage == 'iwannasunksuperbad' && SONG.song.toLowerCase()=='milk')
		{
			switch (curStep)
			{
				//my dumbass looking at gameplay not realizing its totaly fucking random
				case 528:
